...
// ── Criterion 25: Pac-Man acceptance criteria ──
...
test('Pac-Man contains a complete standalone game implementation', () => {
    // Must have style + script (self-contained)
    if (!/<style>[\s\S]*<\/style>/.test(pacman)) throw new Error('No <style> tag');
    if (!/<script>[\s\S]*<\/script>/.test(pacman)) throw new Error('No <script> tag');
});

test('Pac-Man maze dimensions: 28 columns × 31 rows', () => {
    if (!pacman.includes('COLS = 28') || !pacman.includes('ROWS = 31'))
        throw new Error('Maze dimensions 28×31 not found');
});

test('Pac-Man maze has walls, dots, power pellets, ghost house', () => {
    if (!pacman.includes('WALL = 1') || !pacman.includes('DOT = 0') || !pacman.includes('PEL = 2'))
        throw new Error('Maze element types not found');
    if (!pacman.includes('GHOUSE = 4') || !pacman.includes('DOOR = 5'))
        throw new Error('Ghost house types not found');
});
...
test('Pac-Man has proper ghost targeting AI', () => {
    ...
    const hasBlinky = pacman.includes('Blinky') || pacman.includes("id: 0") || pacman.includes('#ff0000');
    const hasPinky = pacman.includes('Pinky') || pacman.includes("id: 1") || pacman.includes('#ffb8ff');
    const hasInky = pacman.includes('Inky') || pacman.includes("id: 2") || pacman.includes('#00ffff');
    const hasClyde = pacman.includes('Clyde') || pacman.includes("id: 3") || pacman.includes('#ffb852');
    if (!hasBlinky || !hasPinky || !hasInky || !hasClyde)
        throw new Error('Ghost targeting AI not fully implemented');
});

test('Pac-Man has full game loop with requestAnimationFrame', () => {
    if (!pacman.includes('requestAnimationFrame'))
        throw new Error('requestAnimationFrame not found');
});

test('Pac-Man has lives, score, high score persistence via localStorage', () => {
    if (!pacman.includes('lives')) throw new Error('Lives not found');
    if (!pacman.includes('score')) throw new Error('Score not found');
    if (!pacman.includes('localStorage')) throw new Error('localStorage not found');
});

test('Pac-Man has arrow key / WASD controls', () => {
    if (!pacman.includes('ArrowUp') || !pacman.includes('ArrowDown') || !pacman.includes('ArrowLeft') || !pacman.includes('ArrowRight'))
        throw new Error('Arrow keys not handled');
    if (!pacman.includes("'w'") || !pacman.includes("'s'") || !pacman.includes("'a'") || !pacman.includes("'d'"))
        throw new Error('WASD keys not handled');
});

test('Pac-Man has ESC pause', () => {
    if (!pacman.includes('Escape') || !pacman.includes('paused'))
        throw new Error('ESC pause not implemented');
});

test('Pac-Man has "Play Again" button', () => {
    if (!pacman.includes('Play Again'))
        throw new Error('Play Again button not found');
});

test('Pac-Man has frightened mode and ghost eating', () => {
    if (!pacman.includes('frightened')) throw new Error('Frightened mode not found');
    if (!pacman.includes('ghostCombo') && !pacman.includes('eatGhost'))
        throw new Error('Ghost eating not implemented');
});

test('Pac-Man has win and game-over states', () => {
    if (!pacman.includes('gameover')) throw new Error('Game over state not found');
    if (!pacman.includes("'win'")) throw new Error('Win state not found');
});
