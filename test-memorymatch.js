...
EMOJIS.forEach(emoji => {
    const count = (memorymatchHtml.match(new RegExp(emoji.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []);
    assert(count.length >= 2, `Emoji ${emoji} appears (pair reference)`);
});

// Also check each emoji individually with String.includes
EMOJIS.forEach(emoji => {
