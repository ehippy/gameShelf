...
// --- gameStore reactive state tests ---

describe('gameStore filtering state', () => {
  let store
  ...
  it('selectedCategory defaults to empty string', () => {
    expect(store.selectedCategory).toBe('')
  })

  it('can set searchQuery on the store', () => {
    store.searchQuery = 'snake'
    expect(store.searchQuery).toBe('snake')
  })

  it('can set selectedCategory on the store', () => {
    store.selectedCategory = 'Arcade'
    expect(store.selectedCategory).toBe('Arcade')
  })

  it('searchQuery and selectedCategory are independent', () => {
    store.searchQuery = 'bird'
    store.selectedCategory = 'Puzzle'
    expect(store.searchQuery).toBe('bird')
    expect(store.selectedCategory).toBe('Puzzle')
  })
})
